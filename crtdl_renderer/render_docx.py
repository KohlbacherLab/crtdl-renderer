...
from .render import (LEGEND, MUSTHAVE_NOTE, NO_CONSTRAINT, Row, block_formula,
                     block_intro, block_rows, cohort_rule, completeness_note,
                     criteria_cards,
                     criteria_head, group_attributes, group_filters, group_names,
                     group_title, has_must_have, version_line)
...
def _criteria_table(doc, block, rows: list[Row]):
    ...
    for cell, h in zip(table.rows[0].cells, criteria_head(block)):
...
def render_docx(q: Query, path: str | Path, today: date | None = None,
    ...
    if q.attribute_groups:
        ...
        table = doc.add_table(rows=1, cols=3)
        table.style = "Light Grid Accent 1"
        for cell, h in zip(table.rows[0].cells, ["Modul / Profil", "Filter", "Attribute"]):
            cell.text = h
        names = group_names(q)
    ...
    for cell, h in zip(legend.rows[0].cells, ["Notation", "Bedeutung"]):
        cell.paragraphs[0].add_run(h).bold = True
    ...
    if q.unresolved:
        ...
        doc.add_paragraph("Für folgende Codes lag keine geprüfte deutsche Bezeichnung vor; "
                          "angezeigt wird die im Export enthaltene Bezeichnung: "
                          + ", ".join(items)).runs[0].italic = True
    doc.save(str(path))
